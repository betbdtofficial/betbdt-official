const Validation = (values, numberCheck, usernameCheck) => {
  // const re = /\S+@\S+\.\S+/;
  let errors = {};
  if (!values.name) {
  ...
  }else if(numberCheck){ // this validation only for message
  ...
  }else if(values.password !== values.password2){ // this validation only for message
